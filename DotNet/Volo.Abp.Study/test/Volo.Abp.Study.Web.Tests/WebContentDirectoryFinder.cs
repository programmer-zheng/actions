...
public static class WebContentDirectoryFinder
{
    public static string? CalculateContentRootFolder()
    {
        ...
        {
            while (!DirectoryContains(directoryInfo.FullName, "Volo.Abp.Study.Web.csproj", SearchOption.AllDirectories))
            {
            ...
            var webProject = Directory.GetFiles(directoryInfo.FullName, string.Empty, SearchOption.AllDirectories)
                .First(filePath => string.Equals(Path.GetFileName(filePath), "Volo.Abp.Study.Web.csproj"));

            return Path.GetDirectoryName(webProject);
        }

        while (!DirectoryContains(directoryInfo.FullName, "Volo.Abp.Study.sln"))
        {
            directoryInfo = directoryInfo.Parent ?? throw new Exception("Could not find content root folder!");
        }

        var webFolder = Path.Combine(directoryInfo.FullName, $"src{Path.DirectorySeparatorChar}Volo.Abp.Study.Web");
